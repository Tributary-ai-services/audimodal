...
const (
	...
	ConflictStrategyVersioning ConflictStrategy = "versioning"
)
...
const (
	...
	DeleteBehaviorTrash   DeleteBehavior = "move_to_trash"
)
...
func NewSyncManager(connector *BoxConnector, localStore storage.LocalStore, config *SyncConfig) *SyncManager {
	if config == nil {
	...
	sm := &SyncManager{
		...
		config:           config,
		tracer:           otel.Tracer("box-sync"),
		syncState:        &DetailedSyncState{},
		changeDetector:   NewChangeDetector(connector),
...
func (sm *SyncManager) StartSync(ctx context.Context, options *SyncOptions) (*SyncResult, error) {
	...
	span.SetAttributes(
		...
		attribute.Bool("use.box.events", sm.config.UseBoxEvents),
	)
...
func (sm *SyncManager) executeDiscoveryPhase(ctx context.Context, options *SyncOptions, result *SyncResult) error {
	...
	// Discover files from Box
	boxFiles, err := sm.discoverBoxFiles(ctx, options)
	if err != nil {
		return fmt.Errorf("box file discovery failed: %w", err)
	}
	...
	result.FilesFound = int64(len(boxFiles) + len(localFiles))
...
func (sm *SyncManager) executeFinalizationPhase(ctx context.Context, result *SyncResult) {
	...
	// Update Box webhook configurations if needed
	if sm.config.UseBoxEvents {
		sm.updateWebhookConfig(ctx)
	}
}
...
func (sm *SyncManager) discoverBoxFiles(ctx context.Context, options *SyncOptions) ([]*BoxFile, error) {
	// This would implement file discovery from Box
	...
	files := []*FileToSync{
		{
			...
			RemotePath:   "/Box/documents/document.pdf",
...
func (sm *SyncManager) downloadFile(ctx context.Context, file *FileToSync) error {
	// Download file from Box to local storage
...
func (sm *SyncManager) uploadFile(ctx context.Context, file *FileToSync) error {
	// Upload file from local storage to Box
	// This would require Box upload API implementation
...
func (sm *SyncManager) deleteFile(ctx context.Context, file *FileToSync) error {
	...
	case DeleteBehaviorTrash:
		...
		trashPath := fmt.Sprintf("/trash/%s", file.FileName)
		return sm.localStore.MoveFile(ctx, file.LocalPath, trashPath)
	default:
...
func (sm *SyncManager) updateWebhookConfig(ctx context.Context) {
	// Update Box webhook configurations for real-time sync
}
...
func NewChangeDetector(connector *BoxConnector) *ChangeDetector {
...
func (cr *ConflictResolver) ResolveConflicts(ctx context.Context) ([]Conflict, error) {
	...
	case ConflictStrategySize:
		// Use larger file
	case ConflictStrategyVersioning:
