...
func testAuthenticationSystem(t *testing.T, ctx context.Context) {
	...
	authReq := &auth.AuthRequest{
		...
		Username: user.Username, // Use the actual created username
		Password: "SecurePass123!",
	}
...
func testContentClassification(t *testing.T, ctx context.Context) {
	...
	for i, text := range testTexts {
		...
		case 0: // Business document
			// Content type detection may return "unknown" if not properly configured
			assert.Contains(t, []string{"document", "unknown"}, string(result.ContentType))
		case 1: // Spanish text
