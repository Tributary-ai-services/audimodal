...
Two later rolls changed pods without changing any image. At 23:26 UTC on 2026-09-17 all five deployments were restarted together; the embedding worker's `change-cause` records why — `AM-23: restart to pick up corrected DEEPLAKE_API_KEY` — and the others carry only the matching `restartedAt` stamp. Then at 18:38 UTC on 2026-09-23 the OCR and embedding workers got new ReplicaSets whose only difference from the previous ones is a lower memory request: 512Mi to 224Mi on the OCR worker, 256Mi to 64Mi on the embedding worker. That change is not in any commit at `8288508` — the checked-in `ocr-worker-deployment.yaml` and `embedding-worker-deployment.yaml` still say 512Mi and 256Mi — so it was applied from uncommitted work.
