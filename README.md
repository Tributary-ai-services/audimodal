...
verified_against: "audimodal@09f2ddc, 2026-09-23"
depth: standard
---
...
**As of 2026-09-23**, verified against commit `09f2ddc` and the running cluster. AudiModal runs on the TAS k3s cluster in namespace `aether-be`, alongside the Aether backend and the DeepLake service, as five deployments:
...
Two later rolls changed pods without changing any image. At 23:26 UTC on 2026-09-17 all five deployments were restarted together; the embedding worker's `change-cause` records why — `AM-23: restart to pick up corrected DEEPLAKE_API_KEY` — and the others carry only the matching `restartedAt` stamp. Then at 18:38 UTC on 2026-09-23 the OCR and embedding workers got new ReplicaSets whose only difference from the previous ones is a lower memory request: 512Mi to 224Mi on the OCR worker, 256Mi to 64Mi on the embedding worker. That change is PR #38, applied from its branch and merged two hours later, at 20:42 UTC, as `09f2ddc`; `kubectl diff` against the two checked-in manifests now shows nothing to change. The memory *limits* were deliberately left at 2Gi and 1Gi. The profile behind the new requests — a p95 working set of 52Mi and 9Mi over 23 hours — was taken while no OCR or embedding job ran, so the requests are set at about four times the idle figure as a scheduling floor, and the limits stay at the ceiling a large PDF or embedding batch needs; the comment above each `resources:` block (`deployments/kubernetes/ocr-worker-deployment.yaml:60-84`, `deployments/kubernetes/embedding-worker-deployment.yaml:90-108`) says so and carries the query to re-profile with once real traffic has run. All three worker pods came back Burstable.

The cluster is no longer one node. A second k3s node, `pinova01`, joined at 14:45 UTC on 2026-09-21, and the scheduler put one of the two OCR replicas on it in that roll; everything else AudiModal runs is still on `um773dev`. One consequence for anyone reading worker logs: from 18:38 to 19:57 UTC on 2026-09-23, Loki's stream for that replica, `audimodal-ocr-worker-759887b85-wlxl5`, held its startup lines and then only `failed to create fsnotify watcher: too many open files`, every five seconds. That is the log collector, not the worker — the same line appears in six other namespaces over the same window, and nothing in the OCR worker's dependency tree imports `fsnotify` — but it means a quiet stream from that replica is not evidence of an idle one.

That roll is the only thing merged since `967a379` that has reached the cluster, and it changed no code. PR #37 and the commits before it changed CI, dependencies, tests, and two defects in code (below, under *Test it*), but no image has been built from them — and as *Build it* explains, the Dockerfile cannot currently build one.
...
The same merge push then failed both workflows, in jobs that run only on a push and had never been reached, because every earlier push died first. Neither failure is in a test, and neither has been fixed since — the push of `09f2ddc` at 20:42 UTC on 2026-09-23 passed the same four test jobs and failed the same way:

- **Docker Build** in Tests and **Build and Push Image** in CI/CD Pipeline (`.github/workflows/test.yml:238-242`, `.github/workflows/ci-cd.yml:151-155`) both stop at `go mod download` with `go: go.mod requires go >= 1.25.13 (running go 1.24.13; GOTOOLCHAIN=local)`, on `8288508` and again on `09f2ddc`. The builder stage is still based on `golang:1.24-alpine` (`Dockerfile:2`); PR #37 moved `go.mod` and the workflows to 1.25 and missed the Dockerfile.
- **Benchmark** (`.github/workflows/test.yml:146-150`, `main` pushes only) runs `make test-bench`, which passes no `-short` (`Makefile:76-78`), so it runs the stress tests the pull-request steps skip. On `8288508`, `TestStressAuthentication` failed after 474 seconds with `"21.094063085010742" is not greater than "1000"`, and `TestMemoryUsage` then hung until `panic: test timed out after 10m0s`. On `09f2ddc`, `TestStressAuthentication` never finished: the package hit the same ten-minute timeout with it still running at 9m54s (`tests/performance_test.go:454`).
...
Without `-short` the suite still fails, in one package, and that is expected. The same command minus `-short`, run at `09f2ddc` on 2026-09-23, passed 24 packages and failed `github.com/jscharber/audimodal/tests` after exactly 600 seconds: `TestStressAuthentication` ran for 436 seconds and failed with `"22.93926982980357" is not greater than "1000"`, then `TestMemoryUsage` hung until `panic: test timed out after 10m0s`. Those are the two tests the nightly stress workflow documents as known-broken, and the same pair that fails the Benchmark job on `main`. Budget ten minutes before you see that result, or pass `-short`.
...
or, from the node's local network, call the `audimodal-nodeport` service on the node address directly (`192.168.68.63:30084`). The port-forward output above is from 2026-08-26; the responses below were captured through the NodePort at 20:45 UTC on 2026-09-23 against image `sec3-auth-ff275f0`, and the two paths reach the same pods, so substitute `localhost:8084` if you forwarded.
...
$ curl -s http://192.168.68.63:30084/health | jq .
{
  ...
  "timestamp": "2026-09-23T20:45:19.112109766Z",
  "version": "1.0.0"
}
```
    ...
    http://192.168.68.63:30084/api/v1/tenants/1d644409-fc3d-4036-bbf5-16c869b5b88c/files | jq .
{
  ...
  "timestamp": "2026-09-23T20:45:19.223700757Z",
  "request_id": "req_1790196319196283608"
}
```
...
- [deployments/kubernetes/](./deployments/kubernetes/) — manifests for the four workers; the OCR and embedding ones match the cluster, the assembler's does not, as *Configuration* describes; the API server's manifest is `aether-be/k8s/audimodal-deployment.yaml`.
